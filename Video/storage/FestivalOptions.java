...
/** FestivalOptions is a frame that lets the user select different volumes for each of the files
 *  associated with the selected video
 *  
 *  VolumePanels are stored in a list, and have one substantiated for each audio file
 * 
...
public class FestivalOptions extends JFrame {
	...
	private JLabel title = new JLabel("Changes to the volume are automatically stored");
	private JLabel instructions = new JLabel("Press \"merge\" to play video from beginning with changes applied");
	...
	/** For the number of audio files that there are, add a new volume slider to the frame */
	...
	private void createSoundtrackVolumeControl() {
		...
		soundtrackVolume.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				Point p = e.getPoint();
				BasicSliderUI sliderUI = (BasicSliderUI) soundtrackVolume.getUI();
