...
/**
 * AddAudio is a frame that gives the user options to choose an audio to add to
 * the selected video It contains a slider that lets the user change where the
 * audio goes, and a checkbox that lets the user decide whether they want to use
 * the slider or the video's current position
 * 
...
public class AddAudioToVideo extends JFrame {
	...
	// Add the components that show what time the user wants to add the audio
	// --------------------------------------------------------------------------------------------------
	...
	/**
	 * Set the label that shows the user the maximum time they can add the
	 * audio, without going over the end of the video
	 */
	...
	/**
	 * Set the label to the left of the slider that shows the exact time the
	 * user has selected
	 */
	...
	/**
	 * Add the details of the chosen audio to the frame so that the user can
	 * easily see what the choice is
	 */
	...
	/**
	 * This method changes the length of the JSlider based on the length of the
	 * audio file the user has selected so that you can't choose a time that
	 * would make the audio go over the end of the video
	 */
	...
	/**
	 * Create the buttons that let the user select: an audio that has already
	 * been created an audio that they will create from text
	 */
	private void createAudioSelectionButtons() {
		...
		// Let the user create a new mp3 file, then once finished, add this as
		// the selected audio
	...
	/**
	 * Add the slider that lets the user choose where they want their chosen
	 * audio to be placed Also add the checkbox that lets the user decide
	 * whether they want to add their audio using the slider or using the
	 * position of the video in the original media player
	 */
	private void addSliderToSelectPosition() {
		...
		// Add the labels that will hold the time value at which the user is
		// adding their audio
		timePanel.add(endTime, BorderLayout.EAST);
		timePanel.add(addTime, BorderLayout.WEST);
		setLabelForLengthOfSlider();

		// Set up the label that holds the final time that the audio will be
		// added at
		...
		// Add the ability to update the label that states the time the slider
		// is on
		timeSlider.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				setPositionNumberLabel();
				// If the slider is enabled, set the value to be the value on
				// the slider
				labelPositionToAddAt.setTimeText(timeSlider.getValue());
			}
	...
	/**
	 * This method creates and adds action listeners for the buttons that let
	 * the user preview where the audio will be placed, once they're happy add
	 * this to the video properly It also lets the user cancel the action which
	 * just removes the frame and no further action is required
	 */
	private void createViewButtons() {
		previewButton = new JButton("Preview");
		// Show the user a small amount of the film, starting from where the
		// user has selected to place the audio
		// ----------------------------------------------------------------------------------------------
		previewButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				CreatePreviewFile preview = new CreatePreviewFile(start
						.getOriginalVideoPath(), audioPath, lf, start
						.getVideoTitle(), position, time);
		...
		addButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// Create a new media object that will be associated with the
				// selected video
				Media m = new Media(audioPath, position, audioLabel.getText(),
						50);
				start.addAudio(m);
